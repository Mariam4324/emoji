...
emojies2.forEach((obj) => {
  const wrapper = document.querySelector(".finder__wrapper");
...
function createCard(obj) {
  ...
  emoji.className = "finder__img";
  emojiName.className = "finder__emoji__name";
  info.className = "finder__emoji__info";

  emoji.innerHTML = obj.symbol;
  emojiName.textContent = obj.title;
  info.textContent = obj.keywords;

  card.append(emoji, emojiName, info);

  return card;
}

// итого: в функции создания карточкм создается только одна карточка, мы ее параметром передаем
// в функцию перебора массива и суем в обертку, 